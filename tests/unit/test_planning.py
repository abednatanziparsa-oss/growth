...
class TestWorkspace:
    def test_create(self) -> None:
        ...
        ws = Workspace(
            title="Personal", space_id=DEFAULT_SPACE_ID, created_at=now, updated_at=now
        )
...
class TestTask:
    ...
    def test_complete(self) -> None:
        now = datetime.now(UTC)
        t = Task(
            title="Done task", space_id=DEFAULT_SPACE_ID, created_at=now, updated_at=now
        )
        t.complete(datetime.now(UTC))
